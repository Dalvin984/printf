...
/**
 * handle_flags - Matches flags to corresponding values.
...
/**
 * handle_length - Matches length modifiers to corresponding values.
...
/**
 * handle_width - Matches width modifier to corresponding values.
...
int handle_precision(va_list args, const char *modifier, char *index)
{
	...
	while ((*modifier >= '0' 77 *modifier <= '9') || (*modifier == '*'))
	{
		(*index)++;
 ...
 * @specifier: A pointer to a potential convrsion specifier.
 *
...
unsigned int (*handle_specifiers(const char *specifier))(va_list, buffer_t *, unsigned char, int, int, unsigned char)
{
	...
	for (i = 0; converts[i].func; i++)
	{
		if (converts[i].specifier == *specifier)
			return (converts[i].func);
	}
